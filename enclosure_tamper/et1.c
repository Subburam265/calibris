...
// Path to your specific logger binary
#define TAMPER_LOG_BIN "/home/pico/calibris/bin/tamper_log_bin/tamper_log"

// --- Function to Log Tamper Data ---
void log_tamper_event() {
    char command[512];
    
    // Construct command: ./tamper_log --type enclosure_tamper --details "..."
    snprintf(command, sizeof(command), 
             "%s --type enclosure_tamper --details \"Enclosure Trigger Detected on GPIO1_C5 (Pin 21)\"", 
             TAMPER_LOG_BIN);
    
    printf("[Trigger] Executing logger: %s\n", command);
    
    int ret = system(command);
    
    if (ret == 0) {
        printf("[Trigger] Tamper event logged successfully to DB.\n");
    } else {
        fprintf(stderr, "[Trigger] Failed to log tamper event (Exit Code: %d)\n", ret);
    }
}

// --- Function to Activate Safe Mode ---
void activate_safe_mode() {
    printf("\n[Trigger] !!! GPIO1_C5 went LOW! Activating Safe Mode... !!!\n");

    // 1. Log the event immediately using your CLI tool
    log_tamper_event();

    // 2. Update config.json to "safe_mode": true
        ...
        if (ret < 0) {
            ...
            if (event.event_type == GPIOD_LINE_EVENT_FALLING_EDGE) {
                ...
                if (gpiod_line_get_value(line) == 0) {
                    ...
                    // Sleep to prevent loop spam if pin stays low
                    sleep(2); 
                }
            }
        }

...
void perform_calibration() {
    // Switch to canonical mode for input
    tcsetattr(STDIN_FILENO, TCSANOW, &old_tio);
    
    printf("\n\n--- CALIBRATION MODE ---\n");
    printf("Place the scale/platform empty and press Enter for tare...");
    getchar();
    ...
    printf("Tare complete. Zero point set.\n\n");
    
    printf("Enter the reference weight in grams: ");
    float reference_weight;
    if (scanf("%f", &reference_weight) != 1) {
    ...
    printf("Place the %g gram weight on the scale and press Enter...", reference_weight);
    getchar();
    
    printf("Measuring... please wait.\n");
    long weight_raw = get_averaged_reading(CALIBRATION_SAMPLES);
    if (weight_raw == -1) {
        printf("Error during calibration measurement. Check connection.\n");
        tcsetattr(STDIN_FILENO, TCSANOW, &new_tio);
        return;
    }
    
    // Calculate scale factor
    float new_scale_factor = (weight_raw - TARE_OFFSET) / reference_weight;
    printf("Calibration reading: %ld\n", weight_raw);
    printf("Calculated scale factor: %g (previous: %g)\n", new_scale_factor, SCALE_FACTOR);
    
    printf("Accept new calibration? (y/n): ");
    char choice = getchar();
    if (choice == 'y' || choice == 'Y') {
        ...
        printf("Calibration cancelled. No changes made.\n");
    }
...
int main() {
    ...
    if (gpiod_line_request(dout_line, &dout_config, 0) < 0) { 
    ...
    while (running) {
        ...
        switch (current_state) {
            ...
            case STATE_LOGGED:
                if (current_weight < WEIGHT_THRESHOLD) {
                    ...
                    printf("\n--- Potential weight change detected, checking stability... ---\n");
                    current_state = STATE_REWEIGHING;
                    clock_gettime(CLOCK_MONOTONIC, &stability_timer_start);
                }
                break;
            case STATE_REWEIGHING:
                if (fabsf(current_weight - logged_weight) <= REWEIGH_THRESHOLD) {
                    printf("\n--- Weight stable, returning to logged state. ---\n");
                    current_state = STATE_LOGGED;
                } else {
                    struct timespec now;
                    clock_gettime(CLOCK_MONOTONIC, &now);
        ...
        printf("\rState: %-10s | Weight: %8.2f g | Raw: %8ld", state_str, current_weight, raw_val);
        fflush(stdout);

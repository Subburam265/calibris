...
// --- Global Variables & All Helper Functions (same as before) ---
...
void cleanup_and_exit(int signum) { running = 0; }
long hx711_read() {
    long count = 0;
    ...
    while (running) {
        ...
        switch (current_state) {
            ...
            case STATE_LOGGED:
                ...
                // --- NEW: Check for added OR subtracted weight ---
            ...
            case STATE_REWEIGHING:
                // --- NEW: Check if weight returned to previous state ---
        ...
        if (kbhit()) {
            char cmd = getchar();
            if (cmd == 't' || cmd == 'T') {
                printf("\rState: %-10s | Taring... please wait...         ", "TARING");
                fflush(stdout);

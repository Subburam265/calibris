 ...
 * - Enclosure Tamper: PERMANENT -> Logs, Locks, and EXITS this program.
 * - Magnetic Tamper: TEMPORARY -> Logs, Pauses, and Resumes (program stays running).
 */
...
// --- ACTION 1: Enclosure Tamper (PERMANENT SAFE MODE + EXIT) ---
void handle_enclosure_tamper() {
    ...
    printf("[Shutdown] Enclosure breach processed. Terminating Monitor.\n");
    running = 0; // <--- This forces the main loop to exit
}
...
// --- Cleanup ---
void cleanup() {
    // Release all lines so other programs (like safe_mode.service) can use them if needed
...
int main() {
    ...
    printf("Starting Integrated Tamper Monitor (v2.1)...\n");
    ...
    while (running) {
        ...
        if (ret > 0) {
                ...
                if (!enclosure_triggered) {
                    if (event.event_type == GPIOD_LINE_EVENT_FALLING_EDGE) {
                        ...
                        if (gpiod_line_get_value(line_enc) == 0) {
                            enclosure_triggered = true;
                            handle_enclosure_tamper();
                            break; // <--- Break main loop immediately
                        }
                    }
                }
            }
            ...
            // --- MAGNETIC EVENT ---
            // Only process if enclosure wasn't just triggered
    ...
    cleanup();
    printf("[Shutdown] Integrated Monitor Exited.\n");
    return 0;
}

...
DEVICE_STATE=$(grep -o '"state":[[:space:]]*"[^"]*"' "$CONFIG_FILE" | cut -d'"' -f4 || true)
...
# --- Define Device-Specific Paths ---
# This targets the specific folder, e.g., /home/pico/calibris/data/1
DEVICE_LOCAL_DIR="$SYNC_DIR/$DEVICE_ID"
MASTER_CSV_PATH="$DEVICE_LOCAL_DIR/$MASTER_CSV_FILENAME"

# Convert State to S3-friendly folder name
DEVICE_STATE_FOLDER=$(echo "$DEVICE_STATE" | tr ' ' '_')

echo "[INFO] Device ID: $DEVICE_ID"
echo "[INFO] Local Directory to Sync: $DEVICE_LOCAL_DIR"
echo "[INFO] Device State: $DEVICE_STATE"
echo "[INFO] S3 Target: $S3_BUCKET/$DEVICE_STATE_FOLDER/$DEVICE_ID/"
...
# Ensure the device directory exists (e.g., ~/calibris/data/1/)
...
# Step 6: Sync to AWS S3 (ALL FILES).
S3_DESTINATION="s3://$S3_BUCKET/$DEVICE_STATE_FOLDER/$DEVICE_ID/"
echo "[INFO] Syncing ALL files from $DEVICE_LOCAL_DIR to $S3_DESTINATION"

# MODIFIED: Removed '--exclude' and '--include' to allow full directory sync
aws s3 sync "$DEVICE_LOCAL_DIR" "$S3_DESTINATION"

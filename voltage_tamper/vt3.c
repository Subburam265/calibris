#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
#include <fcntl.h>
#include <unistd.h>
#include <sys/ioctl.h>
#include <sys/wait.h>
#include <sys/types.h>
#include <linux/i2c-dev.h>
#include <time.h>
#include <signal.h>
#include <string.h>

// I2C Configuration
#define INA219_ADDRESS  0x40
#define I2C_DEVICE      "/dev/i2c-3"

// INA219 Registers
#define INA219_REG_CONFIG       0x00
#define INA219_REG_BUSVOLTAGE   0x02

// Config: 32V Range, 12-bit ADC, Continuous Mode
#define INA219_CONFIG_BVOLTAGERANGE_32V    (0x2000)
#define INA219_CONFIG_BADCRES_12BIT        (0x0180)
#define INA219_CONFIG_MODE_SANDBVOLT_CONT  (0x0007)

// Voltage Monitoring Thresholds
#define REF_VOLTAGE     3.3f
#define TOLERANCE       2.0f
#define MIN_VOLTAGE     (REF_VOLTAGE - TOLERANCE) // 1.3V
#define MAX_VOLTAGE     (REF_VOLTAGE + TOLERANCE) // 5.3V

// External Tool Paths
#define TAMPER_LOG_BIN  "/home/pico/calibris/bin/tamper_log_bin/tamper_log"
#define LOCAL_LOG_FILE  "/var/log/ina219_tamper.log"

static int i2c_fd = -1;
static volatile sig_atomic_t running = 1;

// Call external tamper_log binary
int call_tamper_log_tool(const char *reason) {
    pid_t pid = fork();
   
    if (pid < 0) {
        perror("fork failed");
        return -1;
    }
   
    if (pid == 0) {
        // Child process
        execl(TAMPER_LOG_BIN, 
              TAMPER_LOG_BIN,
              "--type", "voltage_tamper",
              "--details", reason,
              (char *)NULL);
       
        // If we get here, execl failed
        perror("execl failed");
        _exit(127);
    }
   
    // Parent process - non-blocking wait to avoid hanging the sensor loop
    // Note: In a production loop, you might want to handle zombies differently (SIGCHLD)
    int status;
    waitpid(pid, &status, 0); 
    return 0;
}

// Log tampering to file and call external tool
void log_tampering(float voltage) {
    time_t now = time(NULL);
    char timestamp[26];
    ctime_r(&now, timestamp);
    timestamp[24] = '\0'; // Remove newline

    char reason[128];
    snprintf(reason, sizeof(reason), "Voltage %.3fV outside range (%.1f-%.1fV)", 
             voltage, MIN_VOLTAGE, MAX_VOLTAGE);

    fprintf(stderr, "\n[TAMPER ALERT] %s\n", reason);

    // 1. Log to local file
    FILE *log = fopen(LOCAL_LOG_FILE, "a");
    if (log) {
        fprintf(log, "[%s] %s\n", timestamp, reason);
        fclose(log);
    } else {
        perror("Failed to open local log file");
    }

    // 2. Call external binary
    call_tamper_log_tool(reason);
}

// I2C Functions
int ina219_write16(uint8_t reg, uint16_t value) {
    uint8_t buf[3];
    buf[0] = reg;
    buf[1] = (value >> 8) & 0xFF;
    buf[2] = value & 0xFF;

    if (write(i2c_fd, buf, 3) != 3) return -1;
    return 0;
}

int16_t ina219_read16(uint8_t reg) {
    uint8_t buf[2];
    if (write(i2c_fd, &reg, 1) != 1) return -1;
    if (read(i2c_fd, buf, 2) != 2) return -1;
    return (int16_t)((buf[0] << 8) | buf[1]);
}

int ina219_init() {
    i2c_fd = open(I2C_DEVICE, O_RDWR);
    if (i2c_fd < 0) {
        perror("Failed to open I2C bus");
        return -1;
    }

    if (ioctl(i2c_fd, I2C_SLAVE, INA219_ADDRESS) < 0) {
        perror("Failed to acquire bus access");
        close(i2c_fd);
        return -1;
    }

    uint16_t config = INA219_CONFIG_BVOLTAGERANGE_32V |
                      INA219_CONFIG_BADCRES_12BIT |
                      INA219_CONFIG_MODE_SANDBVOLT_CONT;

    return ina219_write16(INA219_REG_CONFIG, config);
}

float get_bus_voltage() {
    int16_t raw_value = ina219_read16(INA219_REG_BUSVOLTAGE);
    return (float)((raw_value >> 3) * 4) * 0.001f;
}

void signal_handler(int sig) {
    running = 0;
}

int main() {
    signal(SIGINT, signal_handler);
    signal(SIGTERM, signal_handler);

    printf("Starting Voltage Monitor with Tamper Logging...\n");
    printf("Target: %.2fV (Safe Range: %.2fV - %.2fV)\n", REF_VOLTAGE, MIN_VOLTAGE, MAX_VOLTAGE);

    if (ina219_init() < 0) return 1;

    while (running) {
        float voltage = get_bus_voltage();

        if (voltage < MIN_VOLTAGE || voltage > MAX_VOLTAGE) {
            log_tampering(voltage);
        } else {
            printf("[OK] Voltage: %.3f V\n", voltage);
        }

        sleep(1);
    }

    if (i2c_fd >= 0) close(i2c_fd);
    printf("\nExiting.\n");
    return 0;
}

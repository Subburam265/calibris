...
// --- NEW GPIO Configuration (Bank 2) ---
#define GPIO_CHIP2 "gpiochip2"
#define PIN_STATUS 0  // GPIO2_A0 is offset 0
...
struct gpiod_chip *chip2 = NULL; // New Chip pointer for Bank 2
...
struct gpiod_line *line_status = NULL; // New Line pointer for Status Pin
...
int gpio_init() {
    ...
    if (!chip) { perror("GPIO Chip 1"); return -1; }
    ...
    line_pwm   = gpiod_chip_get_line(chip, PIN_PWM);

    if (!line_enter || !line_decr || !line_incr || !line_pwm) { fprintf(stderr, "Failed to get lines Bank 1\n"); return -1; }
    ...
    chip2 = gpiod_chip_open_by_name(GPIO_CHIP2);
    if (!chip2) { perror("GPIO Chip 2"); return -1; }

    line_status = gpiod_chip_get_line(chip2, PIN_STATUS);
    if (!line_status) { fprintf(stderr, "Failed to get lines Bank 2\n"); return -1; }

    // Request as Output with Initial Value 1 (HIGH)
    if (gpiod_line_request_output(line_status, "sm_status", 1) < 0) {
        perror("Request Status Output failed"); return -1;
    }
...
void gpio_close() {
    // --- Turn off GPIO2_A0 before closing ---
    if (line_status) { gpiod_line_set_value(line_status, 0); gpiod_line_release(line_status); }

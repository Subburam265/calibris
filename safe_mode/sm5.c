...
#include <linux/gpio.h> // Standard Linux GPIO ioctl
#include <errno.h>
#include <termios.h>
#include <signal.h>
#include <time.h>
...
int i2c_fd = -1;
int gpio_line_fd = -1; // FD for the TARE button
...
void sha1_update(SHA1_CTX *context, const uint8_t *data, size_t len) {
    ...
    if ((j + len) > 63) {
        ...
        for (; i + 63 < len; i += 64) sha1_transform(context->state, &data[i]);
        j = 0;
    } else i = 0;
...
int verify_totp(const char *product_id, const char *token_str) {
    ...
    for (int i = -TOKEN_VALIDITY_WINDOW; i <= TOKEN_VALIDITY_WINDOW; i++) {
        ...
        if (expected_token == input_token) return 1;
    }
    return 0;
}
...
int tare_gpio_init() {
    ...
    // Configured for Active HIGH (3.3V/5V when pressed)
    // Note: Pin must have a Pull-Down resistor to Ground to avoid floating.
...
// Check if TARE button is pressed (returns 1 if pressed, 0 if not)
...
int verify_clearance_token() {
    ...
    if (verify_totp(device_id, token)) {
        lcd_clear(); lcd_set_cursor(0, 0); lcd_string("Access Granted!"); lcd_set_cursor(0, 1); lcd_string("Starting Scale..");
...
int main() {
    ...
    while (1) {
        ...
        if (is_tare_button_pressed()) {
            tare_debounce++;
            if (tare_debounce >= 3) { // Simple debounce

...
int update_config_safe_mode(int enable_safe_mode) {
    ...
    if (strncmp(value_start, "true", 4) == 0) {
        ...
        fprintf(stderr, "[config] Error: Invalid safe_mode value\n");
        free(content);
        return -1;
    }
    ...
    printf("[config] Updated safe_mode = %s\n", new_value);
...
int exit_safe_mode() {
    ...
    if (WIFEXITED(status) && WEXITSTATUS(status) == 0) {
        printf("[service] Started %s successfully.\n", MW7_SERVICE);
        return 0;
    }

    perror("Failed to start measure_weight.service");
    return -1;
}

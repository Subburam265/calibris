...
// Mapping based on Rockchip GPIO1_C (Group C starts at offset 16)
// GPIO1_C1 (Pin 14) -> Offset 17
// GPIO1_C2 (Pin 15) -> Offset 18
// GPIO1_C3 (Pin 16) -> Offset 19
#define PIN_ENTER 17
#define PIN_DECR  18
#define PIN_INCR  19  // Also TARE/Dev Bypass in Idle mode
...
// --- HMAC-SHA1 Implementation (Standard) ---
...
void sha1_transform(uint32_t state[5], const uint8_t buffer[64]) {
    ...
    // Tiny SHA1 implementation macros
...
void sha1_final(uint8_t digest[SHA1_DIGEST_SIZE], SHA1_CTX *ctx) {
    uint8_t finalcount[8], c = 0200;
    for (int i=0; i<8; i++) finalcount[i] = (uint8_t)((ctx->count[(i>=4?0:1)] >> ((3-(i&3))*8)) & 255);
    sha1_update(ctx, &c, 1);
    while ((ctx->count[0] & 504) != 448) { c=0000; sha1_update(ctx, &c, 1); }
    sha1_update(ctx, finalcount, 8);
    for (int i=0; i<SHA1_DIGEST_SIZE; i++) digest[i] = (uint8_t)((ctx->state[i>>2] >> ((3-(i&3))*8)) & 255);
}
...
void hmac_sha1(const uint8_t *key, size_t key_len, const uint8_t *data, size_t data_len, uint8_t *digest) {
    ...
    if (key_len > SHA1_BLOCK_SIZE) { 
        SHA1_CTX ctx; sha1_init(&ctx); sha1_update(&ctx, key, key_len); sha1_final(tk, &ctx);
        key = tk; key_len = SHA1_DIGEST_SIZE;
    }
    ...
    SHA1_CTX ctx;
    sha1_init(&ctx); sha1_update(&ctx, k_ipad, SHA1_BLOCK_SIZE); sha1_update(&ctx, data, data_len); sha1_final(inner, &ctx);
...
// FIXED HMAC-SHA256 Function (Swapped arguments in sha256_final calls)
void hmac_sha256(const uint8_t *key, size_t key_len, const uint8_t *data, size_t data_len, uint8_t *digest) {
    ...
    if (key_len > SHA256_BLOCK_SIZE) { 
        SHA256_CTX ctx; sha256_init(&ctx); sha256_update(&ctx,key,key_len); 
        sha256_final(&ctx, tk); // FIXED: ctx first, buffer second
        key=tk; key_len=SHA256_DIGEST_SIZE; 
    }
    memset(k_ipad,0x36,SHA256_BLOCK_SIZE); memset(k_opad,0x5c,SHA256_BLOCK_SIZE);
    for(size_t i=0;i<key_len;i++) { k_ipad[i]^=key[i]; k_opad[i]^=key[i]; }
    
    SHA256_CTX ctx; 
    sha256_init(&ctx); sha256_update(&ctx,k_ipad,SHA256_BLOCK_SIZE); sha256_update(&ctx,data,data_len); 
    sha256_final(&ctx, inner); // FIXED: ctx first, buffer second
    
    sha256_init(&ctx); sha256_update(&ctx,k_opad,SHA256_BLOCK_SIZE); sha256_update(&ctx,inner,SHA256_DIGEST_SIZE); 
    sha256_final(&ctx, digest); // FIXED: ctx first, buffer second
}
...
// --- LCD Functions ---
void lcd_pulse(int data) {
    if (i2c_fd < 0) return;
    unsigned char buf[] = { data | LCD_E, data & ~LCD_E };
    write(i2c_fd, &buf[0], 1); usleep(500);
    write(i2c_fd, &buf[1], 1); usleep(500);
}
void lcd_send(int val, int mode) {
    int h = val & 0xF0, l = (val << 4) & 0xF0;
    unsigned char b1 = h|mode|LCD_BACKLIGHT; write(i2c_fd, &b1, 1); lcd_pulse(b1);
    unsigned char b2 = l|mode|LCD_BACKLIGHT; write(i2c_fd, &b2, 1); lcd_pulse(b2);
}
void lcd_command(int c) { lcd_send(c, 0); }
void lcd_data(int d)    { lcd_send(d, LCD_RS); }
void lcd_string(const char *s) { while(*s) lcd_data(*s++); }
void lcd_cursor(int c, int r) { int off[]={0,0x40,0x14,0x54}; lcd_command(LCD_SETDDRAMADDR|(c+off[r])); }
void lcd_clear() { lcd_command(LCD_CLEARDISPLAY); usleep(2000); }
int lcd_init() {
    if ((i2c_fd = open(I2C_DEVICE, O_RDWR)) < 0) return -1;
    if (ioctl(i2c_fd, I2C_SLAVE, I2C_ADDR) < 0) { close(i2c_fd); return -1; }
    usleep(50000); lcd_send(0x30,0); usleep(4500); lcd_send(0x30,0); usleep(150); lcd_send(0x30,0); lcd_send(0x20,0);
    lcd_command(LCD_FUNCTIONSET|0x08); lcd_command(LCD_DISPLAYCONTROL|0x04); lcd_command(LCD_ENTRYMODESET|0x02); lcd_clear();
    return 0;
}
void lcd_close_func() { if(i2c_fd>=0) close(i2c_fd); }
...
void cleanup(int s) { lcd_clear(); lcd_string("Shutting Down"); lcd_close_func(); gpio_close(); exit(0); }
...
int main() {
    ...
    if (lcd_init() != 0 || gpio_init() != 0) return 1;
    ...
    lcd_clear(); lcd_string("** SAFE MODE **"); 
    lcd_cursor(0,1); lcd_string("Press Enter...");
    ...
    while(1) {
        ...
        if (state == STATE_IDLE) {
            if (inc && !last_inc) {
                lcd_clear(); lcd_string("DEV BYPASS");
                lcd_cursor(0,1); lcd_string("Access Granted");
                usleep(2000000);
                exit_safe_mode();
                return 0;
            }
            if (ent && !last_ent) {
                ...
                lcd_clear(); lcd_string("Enter Token:");
                lcd_cursor(0,1); lcd_string("0_____");
                lcd_cursor(0,1);
                lcd_command(0x0F);
            }
        } 
            ...
            if (ent && !last_ent) {
                ...
                if (digit_idx < 6) {
                    ...
                    lcd_command(0x0C);
                    lcd_clear(); lcd_string("Verifying...");
                    usleep(500000);

                    if (verify_totp(device_id, token)) {
                        lcd_cursor(0,1); lcd_string("Success!");
                        usleep(1500000);
                        exit_safe_mode();
                        return 0;
                    } else {
                        lcd_cursor(0,1); lcd_string("Invalid Token");
                        usleep(2000000);
                        state = STATE_IDLE;
                        lcd_clear(); lcd_string("** SAFE MODE **"); 
                        lcd_cursor(0,1); lcd_string("Press Enter...");

 ...
 * Modified to mirror Input (GPIO1_C7_d) to Output (GPIO1_C6_d)
 *
...
// Input: GPIO1_C7_d (Existing)
const unsigned int line_offset_input = 23; 

// Output: GPIO1_C6_d (New - Calculated as 23 - 1)
...
struct gpiod_line *line_in = NULL;  // Renamed from 'line' for clarity
struct gpiod_line *line_out = NULL; // New output line
...
int init_gpio(void) {
    ...
    if (!line_out) {
        ...
        // Note: We don't close chip here to keep line_in valid, but in a robust app we might cleanup both
        return -1;
    }

    // Request as output, default value 0 (LOW)
...
void cleanup(void) {
    ...
    if (line_out) {
        // Ensure output is low on exit? Optional.
...
int main(void) {
    ...
    printf("[Init] Initializing GPIOs on %s...\n", chipname);
    printf("       Input:  %u (GPIO1_C7_d)\n", line_offset_input);
    printf("       Output: %u (GPIO1_C6_d)\n", line_offset_output);
    ...
    // Check binaries (code omitted for brevity, identical to original)
    if (access(TAMPER_LOG_BIN, F_OK) != 0) {
    ...
    while (running) {
        ...
        // --- UPDATE OUTPUT PIN ---
        // Mirror the input state to the output pin immediately
        // High (1) -> Output High (1)
        // Low (0)  -> Output Low (0)
        if (line_out) {
